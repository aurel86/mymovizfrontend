...
function Home() {
  ...
  useEffect(() => {
    fetch('https://mymovizbackend-red.vercel.app/movies')
  ...
  const movie = moviesData.map((data, i) => {
    
    const isLiked = likedMovies.some(movie => movie === data.title);
    console.log()
    
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.original_title} overview={data.overview.substring(0, 250) + '...'} poster={`https://image.tmdb.org/t/p/w500/`+ data.poster_path} voteAverage={data.vote_average} voteCount={data.vote_count} />;
